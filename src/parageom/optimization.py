...
StressWrapper = namedtuple(
        "StressWrapper", ["expr", "cells", "fun"]
        )
ModelWrapper = namedtuple(
        "ModelWrapper", ["model", "solution", "dofmap", "modes", "sol_local"],
        defaults=(None, ) * 3
        )
...
def main(args):
    ...
    def evaluate_constraint(wrapped_model, mu, stress_expr, cells, stress_fun):
        ...
        if model.name == "FOM":
            ...
            urb = model.solve(mu)
            reconstruct(urb.to_numpy(), wrapped_model.dofmap, wrapped_model.modes, wrapped_model.sol_local, wrapped_model.solution) # type: ignore
    ...
    opt_fom_result = solve_optimization_problem(
        logger, args, initial_guess, bounds, evaluate_constraint, wrapped_fom, SigmaFom, fom_minimization_data, gradient=False)
    ...
    opt_rom_result = solve_optimization_problem(
            logger, args, initial_guess, bounds, evaluate_constraint, wrapped_rom, SigmaRom, rom_minimization_data, gradient=False)
