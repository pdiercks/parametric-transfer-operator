# Getting started
In this example a heterogeneous beam structure is optimized. The design variables are the Young's Moduli $E_q$, $q=1, \ldots, N$ for the $N$ subdomains. Each subdomain correpsonds to a _unit cell_.
The objective function is the _compliance_ $C(\boldsymbol{u}_{\mu}, \mu) = \boldsymbol{f}_{\mathrm{ext}}^T \boldsymbol{u}_{\mu}$.
