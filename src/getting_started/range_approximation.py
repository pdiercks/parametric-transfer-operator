...
def adaptive_rrf(
    ...
    while maxnorm > testlimit:
        ...
        if draw:
            v = T.generate_random_boundary_data(
                1, distribution=distribution, **sampling_options
            )
            if iscomplex:
                v += 1j * T.generate_random_boundary_data(
                    1, distribution=distribution, **sampling_options
                )
        else:
...
def approximate_range(beam, mu, configuration, distribution="normal"):
    ...
    if transfer_problem.kernel is None:
        ...
        U_orth = orthogonal_part(
            U_in, transfer_problem.kernel, product=range_product, orthonormal=True
        )
    ...
    product = NumpyMatrixOperator(
        product_matrix, source_id=basis_numpy.space.id, range_id=basis_numpy.space.id
    )
...
def main(args):
    ...
    set_defaults(
        {
            "pymor.core.logger.getLogger.filename": beam.log_range_approximation(
    ...
    np.save(
        beam.loc_pod_modes(args.distribution, args.configuration), pod_modes.to_numpy()
    )
