...
def mark_subdomains(fine_grid: str, coarse_domain: mesh.Mesh) -> None:
    ...
    w = fem.Function(W)
    w.vector.array[:] = W.dofmap.list.flatten()
...
def generate_meshes(example) -> None:
    ...
    # create coarse grid
    create_rectangle_grid(0., 10., 0., 1., num_cells=(example.nx, example.ny), recombine=recombine, out_file=example.coarse_grid.as_posix())

    # create unit cell grid
    create_rectangle_grid(0., 1. , 0., 1., num_cells=example.resolution, recombine=recombine, out_file=example.unit_cell_grid.as_posix())
    ...
    # ### oversampling domain
    create_rectangle_grid(0., 3., 0., 1., num_cells=(3, 1), recombine=recombine, out_file=example.coarse_oversampling_grid.as_posix())
