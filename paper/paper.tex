...
The transfer operator $\bm{T}_{\m}: \mathcal{S}\to\mathcal{R}$, maps the boundary data $\bm{g}$ to the solution on the target subdomain $\bm{u}(\m)\big\vert_{\varOmega^{\p}_{\inrm}} = \bm{T}_{\m}(\bm{g}).$
...
\added[id=pd]{Mention additional Neumann problem, data to be added to the final basis. Introduce terminology of spectral basis as part without Neumann}
...
\begin{figure}
    ...
    \caption{Exemplary coarse grid discretization of the global domain $\varOmega^{\m}_{\gl}$, with oversampling domain $\varOmega^{\m}$ in grey (a) and fine grid discretization of the unit cell (b).}\label{fig:oversampling_domain}
\end{figure}
