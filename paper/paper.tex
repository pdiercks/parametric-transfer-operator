...
The approach adopted in this work to facilitate geometrical parametrizations belongs to the class of surface-based deformations~\cite{Botsch2010Polygon} and is based on transformations $\Phi_{\mu}$ that map each material point $\bm{x}_{\mathrm{p}}$ of a parameter-independent reference or parent domain $\varOmega^{\mathrm{p}}$ to a point $\bm{x}_{\mu}$ in the parameter-dependent current or physical domain $\varOmega^{\mu}$\comment[id=pd]{Add figure and reference to figure}.
...
The transformation map $\Phi_{\mu}$ from parent to physical domain $\Phi_{\mu}: \varOmega^{\mathrm{p}}\mapsto\varOmega^{\mu}$ is given by $\bm{x}_{\mu}=\Phi_{\mu}(\bm{x}_{\mathrm{p}}) = \bm{x}_{\mathrm{p}} + \bm{d}(\bm{x}_{\mathrm{p}}; \mu)$, 
% \begin{align}
%     \Phi_{\mu}&: \varOmega^{\mathrm{p}}\mapsto\varOmega^{\mu}\,,\\\bm{x}_{\mu}&=\Phi_{\mu}(\bm{x}_{\mathrm{p}}) = \bm{x}_{\mathrm{p}} + \bm{d}(\bm{x}_{\mathrm{p}}; \mu)\,,
%     \label{eq:trafo}
% \end{align}
with $\bm{d}(\bm{x}_{\mathrm{p}}; \bm{\mu})$ being the transformation displacement field.
\comment[id=pd]{Consistent notation for $\mu$ and its components. Suffix must also be bold then?}
The transformation displacement is determined by solving the following linear elastostatic auxiliary problem.
\begin{align}
    \div{\left(\mathbb{C}\vdot\!\vdot\frac{1}{2}\left(
            \bm{d}\otimes\grad + \grad\otimes\bm{d}
\right)\right)}
            = \bm{0}\,,\quad&\mathrm{in}\;\varOmega^{\mathrm{p}}\,,\\
    \bm{d} = \bm{0}\,,\quad&\mathrm{on}\;\partial\varOmega^{\mathrm{p}}\,,\\
    \bm{d} = \bm{x}_{\mu} - \bm{x}_{\mathrm{p}}\,,\quad&\mathrm{on}\;\partial\varOmega^{\mathrm{p}}_{\mathrm{int}}\,.
    \label{eq:aux}
\end{align}
\comment[id=pd]{Nabla should be explicitly defined?}

\begin{figure}
    \begin{center}
        \includegraphics[width=0.95\textwidth]{./img/transformationmap.pdf}
    \end{center}
    \caption{Mapping.}\label{fig:transformationmap}
\end{figure}
