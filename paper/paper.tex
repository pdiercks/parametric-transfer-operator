...
Each unit cell, \ie{} subdomain $\varOmega^{\p}_i$, is taken as a square of size $a$ with a circular void with radius $R$ and is partitioned in \added{384} isoparametric quadrilateral FEs of polynomial degree two, see~\cref{fig:oversampling_domain}\,(b).
With regard to the geometrical parametrization, we consider the variation of the radius $R$ in each subdomain.
For the global problem this results in a parameter space $\mathcal{P}_{\gl}=[R_{-}, R_{+}]^{10}$, such that a parameter value $\m\in\mathbb{P}_{\gl}$ has parameter components $\m_i\in[R_{-},R_{+}], i=1, \ldots, 10$, with ranges chosen as $R_{-}=0.1a, R_{+}=0.3a$.
...
\begin{itemize}
    \item \added{number of snapshots for each POD}
    \item difference to full order model: only use operators on unit cell domain in the online phase
\end{itemize}
...
The error in the displacements in measured in the energy norm (over the domain $\varOmega^{\p}_{\gl}$) and defined as
\begin{equation}
    e_u(\m) := \frac{\norm{\bm{u}_{\fom} - \bm{u}_{\rom}}}{\norm{\bm{u}_{\fom}}}\,.
    \label{eq:error_u}
\end{equation}
The error in the first principal component of the \textsc{Cauchy} stress is defined as
\begin{equation}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.65\textwidth]{relerr.pdf}
    \end{center}
    \caption{}\label{fig:relerr}
\end{figure}
