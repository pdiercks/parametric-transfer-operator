...
\begin{algorithm}
    \caption{HRRF caption.}%
    \label{algo:hrrf}
    \begin{algorithmic}[1]
        \Function{HeuristicRangeApproximation}{$\bm{T}$, $\texttt{tol}$, $n_t$, $\varepsilon_{\mathrm{algofail}}$, $N_{\mathrm{train}}$, $N_{\mathrm{test}}$}
    \Statex{\textbf{Input:} Operator $\bm{T}_{\bm\mu}$, target tolerance $\texttt{tol}$, number of testvectors $n_t$, maximum failure probability $\varepsilon_{\mathrm{algofail}}$, number of parameter samples $N_{\mathrm{train}}$, number of parameter samples $N_{\mathrm{test}}$}
    \Statex{\textbf{Output:} space $X^n$}

    \State{$\bm{B}$ $\gets\;\emptyset$}\Comment{initialize basis}
    \State{$\bm{S}_{\mathrm{train}}\gets\;\{\bm\mu_1,\ldots,\bm\mu_{N_\mathrm{train}}\}$}\Comment{initialize training set}
    \State{$\bm{S}_{\mathrm{test}}\gets\;\{\bm\mu_1,\ldots,\bm\mu_{N_\mathrm{test}}\}$}\Comment{initialize testing set}
    \State{$\bm{M}\gets\{\bm{T}_{\bm\mu_1}(\bm{g}^1),\ldots,\bm{T}_{\bm\mu_1}(\bm{g}^{n_t}),\ldots,\bm{T}_{\bm\mu_{N_{\mathrm{test}}}}(\bm{g}^1),\ldots,\bm{T}_{\bm\mu_{N_{\mathrm{test}}}}(\bm{g}^{n_t})\}$}\Comment{initialize $N_{\mathrm{test}}\cdot\,n_t$ test vectors}
    ...
    \While{$\left(\max_{t\in \bm{M}} \norm{t}_R\right) \vdot c_{\mathrm{est}} > \texttt{tol}$}
        \State{$\bm\mu_j\gets\mathrm{random.choice}(\bm{S}_{\mathrm{train}})$}
        \State{$\bm{B}\gets \bm{B}\cup\left(\bm{T}_{\bm\mu_j}(\bm{g})\right)$}
        \State{$\bm{B}\gets \mathrm{orthonormalize}\left(\bm{B}\right)$}
        \State{$\bm{M}\gets\{t-P_{\mathrm{span}(\bm{B})}t \mid t\in \bm{M}\}$}\Comment{orthogonalize test vectors to $\mathrm{span}(\bm{B})$}
    \EndWhile{}
    \State{\textbf{return} {$X^n=\mathrm{span}\left(\bm{B}\right)$}}
    \EndFunction%
    \end{algorithmic} 
